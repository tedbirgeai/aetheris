...
// AttachObservability, /metrics ve /readyz'i VERİLEN mux'a kaydeder — ana
// gateway mux'ı için (main.go'da /healthz satırının yanına tek çağrı). Token
// AETHERIS_METRICS_TOKEN (yoksa AETHERIS_ADMIN_TOKEN) env'inden okunur; böylece
// *Server gerekmez. provider canlı telemetri kaynağı (nil olabilir).
func AttachObservability(mux *http.ServeMux, provider MetricsProvider) {
	if provider != nil {
	...
	mux.HandleFunc("/metrics", func(w http.ResponseWriter, r *http.Request) {
		...
		if tok == "" {
			tok = os.Getenv("AETHERIS_ADMIN_TOKEN")
		}
		if q := r.URL.Query().Get("token"); tok != "" && q != tok && r.Header.Get("Authorization") != "Bearer "+tok {
			w.WriteHeader(http.StatusUnauthorized)
			_, _ = w.Write([]byte("yetkisiz\n"))
			return
		}
		w.Header().Set("Content-Type", "text/plain; version=0.0.4; charset=utf-8")
		_, _ = w.Write([]byte(buildMetrics()))
	})
}

// buildMetrics, Prometheus text exposition çıktısını üretir.
